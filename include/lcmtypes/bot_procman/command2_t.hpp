...
int command2_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    this->exec_str.assign(static_cast<const char*>(buf) + offset + pos, __exec_str_len__ - 1);
    ...
    this->command_name.assign(static_cast<const char*>(buf) + offset + pos, __command_name_len__ - 1);
    ...
    this->group.assign(static_cast<const char*>(buf) + offset + pos, __group_len__ - 1);
    pos += __group_len__;
    ...
    for (int a0 = 0; a0 < this->num_options; a0++) {
        ...
        this->option_names[a0].assign(static_cast<const char*>(buf) + offset + pos, __elem_len -  1);
        pos += __elem_len;
    }
    ...
    for (int a0 = 0; a0 < this->num_options; a0++) {
        ...
        if(tlen < 0) return tlen; else pos += tlen;
        if(__elem_len > maxlen - pos) return -1;
        this->option_values[a0].assign(static_cast<const char*>(buf) + offset + pos, __elem_len -  1);
        pos += __elem_len;
    }
