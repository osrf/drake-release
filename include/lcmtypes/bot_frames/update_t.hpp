...
int update_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    char* frame_cstr = const_cast<char*>(this->frame.c_str());
    tlen = __string_encode_array(buf, offset + pos, maxlen - pos, &frame_cstr, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    char* relative_to_cstr = const_cast<char*>(this->relative_to.c_str());
...
int update_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    this->frame.assign(static_cast<const char*>(buf) + offset + pos, __frame_len__ - 1);
    pos += __frame_len__;
    ...
    this->relative_to.assign(static_cast<const char*>(buf) + offset + pos, __relative_to_len__ - 1);
...
{
    uint64_t hash = 0xe81404582309d273LL;
